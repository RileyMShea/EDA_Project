...
Ahh, this transformed quality histogram looks much more normal.
...
measurements or maybe a value that some winemakes aime to have in their wine.
...
a bimodal graph with a couple peaks at .25, and .5.

```{r}
qplot(data= wine, x = residual.sugar, bins = 50)
```

Initialy this residual sugar looks somewhat longtailed, but limiting the x axis
should give us a more normal shape
...
This scale helps use see it's unimodal distribution.
...
Once again applying this scale_x_log10 helps us see the normal, unimodal shape.
...
Could argue this alcohol histogram is longtailed or normal I think.
...
Looks to be a somewhat bidmodal, possibly multi-modal shape here.  
...
so all observations were of dry wines.
...
Only tidying I did was to remove the 'X' field as it was redundant with R's own
row names.
...
Unfortunately this faceted plot isn't very useful since the classes aren't
balanced.
...
description of values provided in the data set description.
...
Ultimately, it seems the bi variate plots we created provided the most value.
...
Here we see a non-linear relationship between Free Sulfur Oxide and Quality.
...
the data set contained information like grape types, wine brand, wine selling price, etc
I feel much more interesting analysis could have been done.  Regardless, we did
see some interesting bi variate relationships between our input variables of
interest and their effect on the output variable 'quality'.

If I were to do further analysis I'd try to wrangle a bigger set, with some of
the aforementioned attributes.  Then I could begin to conclusions on things
